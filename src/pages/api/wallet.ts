...
export const POST: APIRoute = async (context) => {
    try {
        ...
        const inviterUsername = clerkUser?.username || clerkUser?.unsafeMetadata?.username as string || user?.username || 'User';
        ...
        if (action === 'ADD_GOAL_FULL') {
            ...
            return new Response(JSON.stringify({ success: true, goal, id: goal.id }), { status: 200 });
        }
            ...
            await supabase.from('transactions').insert({
                ...
                type: 'SETORAN',
                amount: amount,
                description: `Berhasil menabung untuk ${goal.title}`
            });
